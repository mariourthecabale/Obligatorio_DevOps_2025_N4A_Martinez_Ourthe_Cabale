...
##Función para ver si un usuario fue creado con éxito.
...
function crear_usuarios {
    ...
    ##Mostrar la cantidad de usuarios creados con éxito.
...
function setear_passwd {
    #Archivo de usuario creados con éxito.
    usuarios_creados="./logs/usuarios_creados.txt"
 
    if [ $set_passwd ]; then
        for i in $(cat $usuarios_creados); do
            echo $i:$password | sudo chpasswd 2>/dev/null
        done
    fi
...
while getopts "ic:" opt; do
    case "$opt" in
        i)
            ...
            ;;
        c)
            ##Verificar que se haya pasado el -c como parámetro
